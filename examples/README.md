...
| `ownership.em` | Plans 83/84's `own`/`borrow`/`borrow var` (`spec/OWNERSHIP.md` §2, §10) — a `borrow Counter` read-only reference, a `borrow var Counter` exclusive mutable reference (legal to call a mutating method through it), an `own Counter` ownership transfer (caller may not reuse the argument afterward), a `describe(x: borrow Int64)` case proving plan 84's real by-value pointer+writeback codegen for a primitive `borrow` parameter, and `increment_primitive(n: borrow var Int64)` actually reassigning its own parameter — closing a real, disclosed gap this session found and fixed: `emerald-sema` used to reject `Stmt::Assign` to EVERY parameter unconditionally, so plan 84's already-built writeback codegen for a by-value `borrow var` was unreachable from real `.em` source; now allowed for exactly that one shape, scoped to top-level functions only (methods still keep the original full-erasure passthrough, a separately disclosed remaining gap). `report`/`increment` are demonstrated on separate `Counter` instances — a real, disclosed consequence of §8's lexical-scope model, found once top-level statements started getting checked at all: a `borrow` and a `borrow var` of the SAME binding conflict anywhere in one flat scope, even used sequentially with no real overlap | `10`, `11`, `10`, `21`, `42` |
| `ownership_zero_cost_benchmark.em` | Plan 84's zero-cost proof for a by-value primitive `borrow` parameter: `plain Int64` vs. `borrow Int64`, 20,000×1,000 calls each — timings close but noisier than plan 81's own newtype benchmark (this session's own independent re-runs found a modest, inconsistent 0–9% gap across separate runs, not clean parity — see `history/2026-09-08T174011Z-plan-of-plans.md` row 84 for the full, honest account rather than a blanket "zero-cost" claim) | two close-but-not-identical CPU-time floats (exact values vary by run) |
| `ffi_ownership.em` | Plan 85's `own`/`borrow` typing at an `unsafe extern "C"` boundary (`spec/OWNERSHIP.md` §7) — `strdup` (real glibc, allocates a fresh copy) declared `own CString`; `strstr` (real glibc, points into its `haystack` argument's own storage) declared `borrow CString`, used only inline, never bound to a name (see `ffi_borrow_return_reject.em`'s companion test for the real, specific rejection binding it to a `Let` produces). Both compile to the identical C ABI call either way — the annotation is documentation-as-code, not a codegen change | `owned copy`, `world` |
