...
| `ownership.em` | Plans 83/84's `own`/`borrow`/`borrow var` (`spec/OWNERSHIP.md` §2, §10) — a `borrow Counter` read-only reference, a `borrow var Counter` exclusive mutable reference (legal to call a mutating method through it), an `own Counter` ownership transfer (caller may not reuse the argument afterward), and a `describe(x: borrow Int64)` case proving plan 84's real by-value pointer+writeback codegen for a primitive `borrow` parameter. `report`/`increment` are demonstrated on separate `Counter` instances — a real, disclosed consequence of §8's lexical-scope model, found once top-level statements started getting checked at all: a `borrow` and a `borrow var` of the SAME binding conflict anywhere in one flat scope, even used sequentially with no real overlap | `10`, `11`, `10`, `21` |
